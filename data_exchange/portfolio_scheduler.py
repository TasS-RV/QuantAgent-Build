"""
Scheduled portfolio analysis → Telegram.
...
-----
    ...
    python data_exchange/portfolio_scheduler.py --source t212 --once

    # hourly loop over a fixed watchlist, print only (no Telegram)
    python data_exchange/portfolio_scheduler.py --source watchlist \\
        --symbols AAPL NVDA BTC-USD --interval 3600 --no-telegram

    # dry run with the mock portfolio (no keys needed)
    python data_exchange/portfolio_scheduler.py --source mock --once --no-telegram
"""
...
# Allow running this file directly as a script OR importing it as a package module.
# Either way, siblings in this directory (trading212_client, Tele_bot) are found.
_DIR = Path(__file__).resolve().parent
if str(_DIR) not in sys.path:
    sys.path.insert(0, str(_DIR))
# Also ensure the repo root is on the path so quant_signal etc. are importable.
_ROOT = _DIR.parent
...
def holdings_from_t212(use_mock: bool = False) -> List[Holding]:
    ...
    return [
        Holding(symbol=p.yf_symbol, entry_price=p.average_price,
                quantity=p.quantity, is_etf=p.is_etf)
        for p in positions
    ]
...
# ─────────────────────────────────────────────────────────────────────────────
#  Data fetch (default: yfinance) — injectable for tests
# ─────────────────────────────────────────────────────────────────────────────
...
def analyse_holdings(
    ...
    """
    Run the deterministic quant decision for each holding. Returns a list of dicts.

    If sentiment_weight > 0 a FinBERT sentiment overlay is blended into each
    ...
    for h in holdings:
        try:
            ...
            dec = compute_quant_decision(
                h.symbol, kline, entry_price=h.entry_price, allow_short=allow_short
            )
...
def format_telegram_message(results: List[dict]) -> str:
    ...
    for r in results:
        ...
        pnl = (
            f" | PnL {r['unrealized_pnl_pct']:+.1f}%"
            if r.get("unrealized_pnl_pct") is not None else ""
        )
        lines.append(
            f"{_ICON.get(r['decision'], '•')} *{r['symbol']}* — *{r['decision']}* "
            f"(sig {r['combined_signal']:+.2f}){etf}\n"
            f"   px {r['current_price']} → 🎯 {r['target_price']} "
            f"| 🛡️ {r['stop_loss']} | R:R {r['risk_reward_ratio']}{pnl}"
        )
    return "\n".join(lines)
...
def send_telegram(message: str) -> bool:
    ...
    resp = requests.post(
        url,
        json={"chat_id": keys.get("chat_id"), "text": message, "parse_mode": "Markdown"},
        timeout=15,
    )
...
def run_once(
    ...
    results = analyse_holdings(
        holdings, fetch_fn, lookback_days, interval, allow_short,
...
def run_scheduler(
    get_holdings: Callable[[], List[Holding]],
...
def main():
    p = argparse.ArgumentParser(
        description="Scheduled portfolio analysis → Telegram.",

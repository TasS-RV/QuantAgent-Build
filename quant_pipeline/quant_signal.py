...
LangGraph runtime (e.g. by the hourly portfolio scheduler and any backtest
validation). Given OHLC kline data it computes:
...
def _talib_signals(kline_data: dict):
    ...
    TA-Lib-backed signals can't be computed — caller falls back to trend-only.
    """
    global _TALIB_WARNED
    try:
        from indicator_agent import quantify_indicators_from_kline
        from quant_pipeline.quant_nodes import quantify_candlestick_patterns
        ...
        if not _TALIB_WARNED:
            warnings.warn(
                "TA-Lib indicator/pattern signals unavailable — using "
